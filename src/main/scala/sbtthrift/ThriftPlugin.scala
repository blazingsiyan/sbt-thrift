...
object ThriftPlugin extends AutoPlugin {
  ...
  def compileThrift(
    ...
    val doIt = FileFunction.cached(cache, inStyle = FilesInfo.lastModified, outStyle = FilesInfo.exists) { files =>
      ...
      Process(s"""mv $outputDir/gen-java $outputDir/main""").!
      (outputDir / "main" ** s"*.$language").get.toSet
    }
  ...
  object autoImport {
    ...
    val thriftSourceDir =
      SettingKey[File]("source-directory", "Source directory for thrift files. Defaults to src/main/thrift")
    ...
    val thriftJavaOptions =
      SettingKey[Seq[String]]("thrift-java-options", "additional options for java thrift generation")
    ...
    val thriftRubyOptions =
      SettingKey[Seq[String]]("thrift-ruby-options", "additional options for ruby thrift generation")
    ...
    val thriftPythonOptions =
      SettingKey[Seq[String]]("thrift-python-options", "additional options for python thrift generation")
    ...
    val thriftDelphiOptions =
      SettingKey[Seq[String]]("thrift-delphi-options", "additional options for delphi thrift generation")
  ...
  val thriftSettings: Seq[Setting[_]] = inConfig(Thrift)(
    Seq(
      thrift := "thrift",
      thriftSourceDir <<= sourceDirectory { _ / "main" / "thrift" },
      thriftJavaEnabled := true,
      thriftJavaOptions := Seq(),
      thriftOutputDir <<= sourceManaged { _ / "" },
      thriftGenerateJava <<= (thriftJavaEnabled, thriftSourceDir, thriftOutputDir, thrift, thriftJavaOptions, streams)
        .map { (te, tsd, tod, t, to, s) =>
          if (te) {
      ...
      thriftGenerateJs <<= (thriftJsEnabled, thriftSourceDir, thriftJsOutputDir, thrift, thriftJsOptions, streams)
        .map { (te, tsd, tod, t, to, s) =>
          if (te) {
      ...
      thriftGenerateRuby <<= (thriftRubyEnabled,
                              thriftSourceDir,
                              thriftRubyOutputDir,
                              thrift,
                              thriftRubyOptions,
                              streams).map { (te, tsd, tod, t, to, s) =>
        if (te) {
      ...
      thriftGeneratePython <<= (thriftPythonEnabled,
                                thriftSourceDir,
                                thriftPythonOutputDir,
                                thrift,
                                thriftPythonOptions,
                                streams).map { (te, tsd, tod, t, to, s) =>
        if (te) {
      ...
      thriftGenerateDelphi <<= (thriftDelphiEnabled,
                                thriftSourceDir,
                                thriftDelphiOutputDir,
                                thrift,
                                thriftDelphiOptions,
                                streams).map { (te, tsd, tod, t, to, s) =>
        if (te) {
    ...
    watchSources <++= thriftSourceDir.map { (tdir) =>
      (tdir ** "*").get
    },
